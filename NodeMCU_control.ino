...
void setup() {
    ...
    while (wifimode) {
        server.handleClient(); // waiting for connecting to AP ;
        delay(10);
    }
    ...
    myservo.attach(13); // Attach the servo on D7 (GPIO13)
...
int servo_Signal = 0;
int buzzer_Signal = 0;
String servo_result = "0";
String buzzer_result = "0";
...
void loop() {
    ...
    if (millis() - cycleTimestamp > 200) {
        ...
        // 將距離數據推送到 IoTtalk
        push("A0", String(distance));

        //控制伺服馬達
        servo_result = pull("D7"); // 從 IoTtalk 獲取 D7 的輸入值
        if (servo_result != "___NULL_DATA___") {
            Serial.println("D7: " + servo_result);
            if(servo_result.toFloat() > 0.7) servo_Signal = 1;
            else if(servo_result.toFloat() < 0.3) servo_Signal = -1;
            else servo_Signal = 0;
            Serial.println(servo_Signal);
        }
        myservo.detach();
        // 根據 D7 的輸入值控制伺服馬達 -1 : 開燈  1 : 關燈
        if(servo_Signal == -1){
        ...
        myservo.detach();
        buzzer_result = pull("D6"); // 從 IoTtalk 獲取 D7 的輸入值
        if (buzzer_result != "___NULL_DATA___") {
            Serial.println("D6: " + buzzer_result);
            if(buzzer_result.toFloat() > 0.0005) buzzer_Signal = 1;
            else buzzer_Signal = 0;
            Serial.println(buzzer_Signal);
        }
        if(buzzer_Signal == 1){
          digitalWrite(buzzerPin, LOW); // 開啟蜂鳴器
          delay(400);
        }
        digitalWrite(buzzerPin, HIGH); // 關閉蜂鳴器
        delay(100);
